...
class SupportTracker:
    ...
    def get_dependent_objects(self, supporting_id: str) -> Set[str]:
        """Get all objects that depend on the given supporting object."""
...
class SupportSystem:
    ...
    def _find_supporting_object(self, object_id: str, node) -> Optional[str]:
        ...
        for other_id, other_node in self.graph.nodes.items():
            ...
            if self._is_object_supported_by(object_bbox, other_bbox):
                # Choose the highest supporter (in case of stacking)
                if other_bbox.top_level > best_support_level:
    ...
    def _is_object_supported_by(self, object_bbox: BoundingBox, potential_supporter_bbox: BoundingBox) -> bool:
        ...
        x_overlap = (abs(object_bbox.center[0] - potential_supporter_bbox.center[0])
                    < (object_bbox.size[0]/2 + potential_supporter_bbox.size[0]/2))
        y_overlap = (abs(object_bbox.center[1] - potential_supporter_bbox.center[1])
                    < (object_bbox.size[1]/2 + potential_supporter_bbox.size[1]/2))
        ...
        return abs(actual_z - expected_z) < z_tolerance
    ...
    def handle_object_removal(self, removed_object_id: str) -> Tuple[List[str], List[Tuple[str, Tuple[float, float, float]]]]:
        ...
        # Get all objects that were depending on the removed object
        dependent_objects = self.support_tracker.remove_object_completely(removed_object_id)

        if not dependent_objects:
            return [], []

        # Apply gravity to dependent objects
        moved_objects = self.gravity_simulator.apply_gravity_with_collision_detection(
            list(dependent_objects)
        )

        return list(dependent_objects), moved_objects
